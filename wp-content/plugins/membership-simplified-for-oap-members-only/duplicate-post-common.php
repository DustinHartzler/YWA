 ...
 */
if ( ! function_exists('duplicate_post_get_copy_user_level')) {
...
function duplicate_post_clone_post_link( $link = null, $before = '', $after = '', $id = 0 ) {
	...
	return;
	if ( null === $link )
	$link = __('Copy to a new draft', DUPLICATE_POST_OAP_DOMAIN);
	$post_type_obj = get_post_type_object( $post->post_type );
	$link = '<a class="post-clone-link" href="' . $url . '" title="'
	. esc_attr(__("Copy to a new draft", DUPLICATE_POST_OAP_DOMAIN))
	.'">' . $link . '</a>';
...
function duplicate_post_admin_bar_render() {
	...
	&& ( $post_type_object->show_ui || 'attachment' == $current_object->post_type ) )
	{
		...
		'parent' => 'edit',
        'id' => 'new_draft',
        'title' => __("Copy to a new draft", DUPLICATE_POST_OAP_DOMAIN),
