 ...
 * Plugin Default
 */
add_action('admin_init','duplicate_post_plugin_default');
function duplicate_post_plugin_default() {
...
function duplicate_post_make_duplicate_link_row($actions, $post) {
	if ($post->post_type == 'oaplesson'){
	if (duplicate_post_is_current_user_allowed_to_copy()) {
		$actions['clone'] = '<a href="'.duplicate_post_get_clone_post_link( $post->ID , 'display', false).'" title="'
		. esc_attr(__("Clone this item", DUPLICATE_POST_OAP_DOMAIN))
		. '">' .  __('Duplicate', DUPLICATE_POST_OAP_DOMAIN) . '</a>';
	}
	}
	return $actions;
}
...
<div id="duplicate-action">
	<a class="submitduplicate duplication"
		href="<?php echo duplicate_post_get_clone_post_link( $_GET['post'] ) ?>"><?php _e('Copy to a new draft', DUPLICATE_POST_OAP_DOMAIN); ?>
	</a>
</div>
		<?php
	}
...
function duplicate_post_save_as_new_post($status = ''){
	if (! ( isset( $_GET['post']) || isset( $_POST['post'])  || ( isset($_REQUEST['action']) && 'duplicate_post_save_as_new_post' == $_REQUEST['action'] ) ) ) {
		wp_die(__('No post to duplicate has been supplied!', DUPLICATE_POST_OAP_DOMAIN));
	}
	...
	if (isset($post) && $post!=null) {
		...
		wp_die(esc_attr(__('Copy creation failed, could not find original:', DUPLICATE_POST_OAP_DOMAIN)) . ' ' . $id);
	}
}
/**
...
function duplicate_post_copy_post_meta_info($new_id, $post) {
	...
	$mediaLists = $wpdb->get_results( "SELECT * FROM $media_table WHERE postID = '".$post->ID."'" );
if($mediaLists){
foreach ( $mediaLists as $mediaList ) 
{
$listing_id= $mediaList->recordListingID;
	$oapmedianame= $mediaList->recordText;
	$filename= $mediaList->fileName;
$data= array( 'postId'=>$new_id,	
			  'recordListingID'=>$listing_id,
...
$download_table = $wpdb->prefix . "download_listing";
	$downloadLists = $wpdb->get_results( "SELECT * FROM $download_table WHERE postID = '".$post->ID."'" );
if($downloadLists){
foreach ( $downloadLists as $downloadList ) 
{
$listing_id= $downloadList->recordListingID;
	$oapmedianame= $downloadList->recordText;
	$filename= $downloadList->fileName;
$data= array( 'postId'=>$new_id,	
			  'recordListingID'=>$listing_id,
		      'recordText' =>$oapmedianame,
			  'fileName'=> $filename
								        );
		      $wpdb->insert( $download_table, $data, $format=null );
...
function duplicate_post_add_plugin_links($links, $file) {
	if ( $file == plugin_basename(dirname(__FILE__).'/duplicate-post.php') ) {
		$links[] = '<a href="http://lopo.it/duplicate-post-plugin">' . __('Donate', DUPLICATE_POST_OAP_DOMAIN) . '</a>';
		$links[] = '<a href="http://lopo.it/duplicate-post-plugin">' . __('Translate', DUPLICATE_POST_OAP_DOMAIN) . '</a>';
	}
	return $links;
}
?>